...
import type { ForwardVix25Dist52AuditReport } from '../types/forwardValidation';
import { Card } from './ui/Card';
import { theme } from '../theme';

type Props = {
  report: ForwardVix25Dist52AuditReport | null;
  loading?: boolean;
};

export function ForwardVix25Dist52AuditPanel({ report, loading }: Props) {
  if (loading) {
    return (
      <Card style={styles.card}>
        <Text style={styles.title}>VIX≥25 52週区分</Text>
  ...
  return (
    <Card style={styles.card}>
      <Text style={styles.title}>VIX≥25 52週乖離</Text>
