...
import type { BayesianAllocationInput, BayesianAllocationReport } from '../types/bayesianAllocation';
import type { QuantDataSource } from '../types/quantValidation';
import { runBayesianAllocation } from './bayesianAllocationEngine';
...
export async function runRobustBayesianAllocation(params: {
  apiKey: string;
  ...
  maxSymbols?: number;
}): Promise<BayesianAllocationReport> {
  const holdings = params.portfolio.filter((p) => p.shares > 0);
  if (holdings.length === 0) {
    throw new Error('保有銘柄がありません。ベイズ配分には1銘柄以上必要です。');
  }
  ...
  const input: BayesianAllocationInput = {
    symbols,
    markets,
    returnMatrix,
    currentWeightsPct,
  ...
  return runBayesianAllocation(input, dataSource);
}

export async function runDemoBayesianAllocation(params: {
  apiKey: string;
  regime: MarketRegimeResult;
}): Promise<BayesianAllocationReport> {
