...
const deviceConnected = /\bdevice\b/m.test(adbOut.split('\n').slice(1).join('\n'));
const commitHash = execSync('git rev-parse --short HEAD', { encoding: 'utf8' }).trim();

const body = `# Device Verify v44 — testID / Verification Helper Report

- **Overall (code-level)**: PASS — testIDs and verification helpers implemented
- **Overall (device E2E)**: **保留** — ${deviceConnected ? '端末接続あり（本レポート時点でE2E再実行は未実施）' : '**実機未接続** — 最終 PASS 判定は実機E2E後'}
- **Commit (pre-push)**: \`${commitHash}\`
...
fs.writeFileSync(
  path.join('docs', 'review', 'DEVICE_VERIFY_V44_TESTID_REPORT.md'),
  body,
  'utf8',
);
console.log('Wrote report, deviceConnected=', deviceConnected);
