/**
 * npx vitest run tests/unit/forwardValidationVixThresholdSweepAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditVixThresholdSweep } from '../../src/services/forwardValidation/forwardValidationVixThresholdSweepAudit';
...
describe('forwardValidationVixThresholdSweepAudit', () => {
  it('returns 9 threshold rows in descending count order', () => {
    ...
    const report = auditVixThresholdSweep({ bundle });
    expect(report.rows).toHaveLength(9);
    for (let i = 1; i < report.rows.length; i++) {
      expect(report.rows[i]!.tradeCount).toBeLessThanOrEqual(report.rows[i - 1]!.tradeCount);
    }
  });
});
