/**
 * npx vitest run tests/unit/forwardValidationMacdExclSpringAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditMacdExclSpring } from '../../src/services/forwardValidation/forwardValidationMacdExclSpringAudit';
...
describe('forwardValidationMacdExclSpringAudit', () => {
  it('excluded + remaining equals macd cohort', () => {
    ...
    const report = auditMacdExclSpring({ bundle });
    expect(report.excludedCount + report.remainingCount).toBe(report.macdCohortCount);
  });
});
