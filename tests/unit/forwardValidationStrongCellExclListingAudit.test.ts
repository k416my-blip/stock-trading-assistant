/**
 * npx vitest run tests/unit/forwardValidationStrongCellExclListingAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditStrongCellExclListing } from '../../src/services/forwardValidation/forwardValidationStrongCellExclListingAudit';
...
describe('forwardValidationStrongCellExclListingAudit', () => {
  it('returns rows with required fields', () => {
    ...
    const report = auditStrongCellExclListing({ bundle });
    expect(report.rows.length).toBe(report.remainingCount);
    for (const r of report.rows) {
      expect(r.signalDate).toBeTruthy();
      expect(r.symbol).toBeTruthy();
      expect(typeof r.returnPct).toBe('number');
      expect(typeof r.holdDays).toBe('number');
      expect(typeof r.macdHistPct).toBe('number');
      expect(typeof r.adx14).toBe('number');
      expect(typeof r.dist52wPct).toBe('number');
    }
  });
});
