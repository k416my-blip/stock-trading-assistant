/**
 * npx vitest run tests/unit/forwardValidationVix25SubBucketAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditVix25SubBuckets } from '../../src/services/forwardValidation/forwardValidationVix25SubBucketAudit';
...
describe('forwardValidationVix25SubBucketAudit', () => {
  it('returns 3 buckets summing to cohort count', () => {
    ...
    const report = auditVix25SubBuckets({ bundle });
    expect(report.buckets).toHaveLength(3);
    expect(report.buckets.reduce((s, b) => s + b.tradeCount, 0)).toBe(report.cohortCount);
  });
});
