/**
 * npx vitest run tests/unit/forwardValidationFiveFactorSweepAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditFiveFactorSweep } from '../../src/services/forwardValidation/forwardValidationFiveFactorSweepAudit';
...
describe('forwardValidationFiveFactorSweepAudit', () => {
  it('returns 5 singles, 10 pairs, 10 triples', () => {
    ...
    const report = auditFiveFactorSweep({ bundle });
    expect(report.singles).toHaveLength(5);
    expect(report.pairs).toHaveLength(10);
    expect(report.triples).toHaveLength(10);
    expect(report.allRows).toHaveLength(25);
  });
});
