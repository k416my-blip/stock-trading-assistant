/**
 * npx vitest run tests/unit/forwardValidationSpyDownClusterAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditSpyDownCluster } from '../../src/services/forwardValidation/forwardValidationSpyDownClusterAudit';
...
describe('forwardValidationSpyDownClusterAudit', () => {
  it('auditSpyDownCluster splits down vs other', () => {
    ...
    const report = auditSpyDownCluster({ bundle });
    expect(report.spyDown.tradeCount + report.other.tradeCount).toBe(report.totalTrades);
    expect(report.clusterMonths).toHaveLength(3);
  });
});
