/**
 * npx vitest run tests/unit/forwardValidationVixSpyBucketAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditVixSpyBuckets } from '../../src/services/forwardValidation/forwardValidationVixSpyBucketAudit';
...
describe('forwardValidationVixSpyBucketAudit', () => {
  it('returns 3 vix and 4 spy bucket rows', () => {
    ...
    const report = auditVixSpyBuckets({ bundle });
    expect(report.vixSection.rows).toHaveLength(3);
    expect(report.spySection.rows).toHaveLength(4);
    const vixSum = report.vixSection.rows.reduce((s, r) => s + r.tradeCount, 0);
    expect(vixSum).toBe(report.vixSection.classifiedCount);
  });
});
