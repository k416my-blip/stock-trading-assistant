/**
 * npx vitest run tests/unit/forwardValidationApril2025ExcludeAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditApril2025Exclusion } from '../../src/services/forwardValidation/forwardValidationApril2025ExcludeAudit';
...
describe('forwardValidationApril2025ExcludeAudit', () => {
  it('reduces count when excluding 2025-04', () => {
    ...
    const report = auditApril2025Exclusion({ bundle });
    expect(report.remainingCount).toBeLessThanOrEqual(report.totalTrades);
    expect(report.overall.before.tradeCount).toBe(report.totalTrades);
    expect(report.overall.after.tradeCount).toBe(report.remainingCount);
  });
});
