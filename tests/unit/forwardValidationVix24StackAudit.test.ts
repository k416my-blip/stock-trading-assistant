/**
 * npx vitest run tests/unit/forwardValidationVix24StackAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditVix24Stack } from '../../src/services/forwardValidation/forwardValidationVix24StackAudit';
...
describe('forwardValidationVix24StackAudit', () => {
  it('returns 8 stack rows with s1 count >= s8', () => {
    ...
    const report = auditVix24Stack({ bundle });
    expect(report.rows).toHaveLength(8);
    expect(report.rows[0]!.tradeCount).toBeGreaterThanOrEqual(report.rows[7]!.tradeCount);
  });
});
