/**
 * npx vitest run tests/unit/forwardValidationLoserFeatureAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditLoserFeatures } from '../../src/services/forwardValidation/forwardValidationLoserFeatureAudit';
...
describe('forwardValidationLoserFeatureAudit', () => {
  it('returns compare rows for loser feature audit', () => {
    ...
    const report = auditLoserFeatures({ bundle });
    expect(report.compareRows.length).toBeGreaterThan(0);
    expect(report.lossCount).toBeGreaterThanOrEqual(0);
  });
});
