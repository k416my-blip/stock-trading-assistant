/**
 * npx vitest run tests/unit/forwardValidationVixSpyFourGroupAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditVixSpyFourGroups } from '../../src/services/forwardValidation/forwardValidationVixSpyFourGroupAudit';
...
describe('forwardValidationVixSpyFourGroupAudit', () => {
  it('returns 4 groups summing to total trades', () => {
    ...
    const report = auditVixSpyFourGroups({ bundle });
    expect(report.groups).toHaveLength(4);
    expect(report.groups.reduce((s, g) => s + g.tradeCount, 0)).toBe(report.totalTrades);
  });
});
