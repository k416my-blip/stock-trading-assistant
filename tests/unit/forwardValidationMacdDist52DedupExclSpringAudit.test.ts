/**
 * npx vitest run tests/unit/forwardValidationMacdDist52DedupExclSpringAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditMacdDist52DedupExclSpring } from '../../src/services/forwardValidation/forwardValidationMacdDist52DedupExclSpringAudit';
...
describe('forwardValidationMacdDist52DedupExclSpringAudit', () => {
  it('excluded + remaining event counts sum to total events', () => {
    ...
    const report = auditMacdDist52DedupExclSpring({ bundle });
    expect(report.excludedEventCount + report.remainingEventCount).toBe(report.totalEventCount);
  });
});
