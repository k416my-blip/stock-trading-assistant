/**
 * npx vitest run tests/unit/forwardValidationRegimePerformanceAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import {
  auditRegimePerformance,
  classifyRegimeGroup,
} from '../../src/services/forwardValidation/forwardValidationRegimePerformanceAudit';
...
describe('forwardValidationRegimePerformanceAudit', () => {
  it('classifyRegimeGroup maps buckets to four groups', () => {
    expect(classifyRegimeGroup('up')).toBe('up');
    expect(classifyRegimeGroup('down')).toBe('down');
    expect(classifyRegimeGroup('sideways_shallow')).toBe('sideways_shallow');
    expect(classifyRegimeGroup('sideways_deep')).toBe('sideways');
    expect(classifyRegimeGroup('unknown')).toBeNull();
  });

  it('auditRegimePerformance returns four groups', () => {
    ...
    const report = auditRegimePerformance({ bundle });
