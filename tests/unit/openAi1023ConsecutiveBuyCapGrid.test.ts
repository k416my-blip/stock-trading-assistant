/**
 * 1023.KL — ベストルール 連続buy上限総当たり（出来高・ATR固定）
 * npx vitest run tests/unit/openAi1023ConsecutiveBuyCapGrid.test.ts
 */
...
const CONSECUTIVE_BUY_CAPS = [4, 5, 6, 7, 8, 9, 10] as const;
...
function passesRule(
  ...
  if (consec > consecutiveBuyMax) return false;
  return true;
}
...
function buildGridRow(consecutiveBuyMax: number, rows: BuyRow[]): GridRow {
  ...
  for (const r of evaluated) {
    const pred = passesRule(r.volumeSurgeRatio, r.atrPct, r.consecutiveBuyNumber, consecutiveBuyMax);
  ...
  const predictedPositive = evaluated.filter((r) =>
    passesRule(r.volumeSurgeRatio, r.atrPct, r.consecutiveBuyNumber, consecutiveBuyMax),
  );
...
describe('1023.KL consecutive buy cap grid', () => {
  it('writes consecutive buy 4-10 grid JSON', async () => {
    ...
    const grid = CONSECUTIVE_BUY_CAPS.map((cap) => buildGridRow(cap, buyRows));
    const current = grid.find((g) => g.consecutiveBuyMax === 8)!;
    ...
    const report = {
      methodologyJa: {
        ...
        variable: '連続buy上限（<= consecutiveBuyMax）',
        universe: `buy ${buyRows.length}件`,
      },
      grid,
      currentBestRuleConsec8: current,
      ranking: { byF1: bestF1 },
      insightJa: [
        `現行 連続buy<=8: 件数${current.count} TP${current.TP} FP${current.FP} F1=${current.f1}`,
        `F1最大: 連続buy<=${bestF1.consecutiveBuyMax} (F1=${bestF1.f1}, 件数${bestF1.count})`,
      ],
    };

    const out = path.join(process.cwd(), 'scripts', 'openai-1023-consecutive-buy-cap-grid.json');
