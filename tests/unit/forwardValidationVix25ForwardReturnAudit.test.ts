/**
 * npx vitest run tests/unit/forwardValidationVix25ForwardReturnAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditVix25ForwardReturns } from '../../src/services/forwardValidation/forwardValidationVix25ForwardReturnAudit';
...
describe('forwardValidationVix25ForwardReturnAudit', () => {
  it('returns 4 horizon rows', () => {
    ...
    const report = auditVix25ForwardReturns({ bundle });
    expect(report.horizons).toHaveLength(4);
    expect(report.horizons.map((h) => h.horizonDays)).toEqual([1, 3, 5, 10]);
  });
});
