/**
 * npx vitest run tests/unit/forwardValidationLoserCompleteAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditLoserComplete } from '../../src/services/forwardValidation/forwardValidationLoserCompleteAudit';
...
describe('forwardValidationLoserCompleteAudit', () => {
  it('lists all losing trades', () => {
    ...
    const report = auditLoserComplete({ bundle });
    expect(report.loserRows.length).toBe(report.lossCount);
    expect(report.winCount + report.lossCount).toBeLessThanOrEqual(report.totalTrades);
  });
});
