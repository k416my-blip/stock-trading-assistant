/**
 * npx vitest run tests/unit/forwardValidationDownDist10WalkForwardAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditDownDist10WalkForward } from '../../src/services/forwardValidation/forwardValidationDownDist10WalkForwardAudit';
...
describe('forwardValidationDownDist10WalkForwardAudit', () => {
  it('returns three walk-forward periods', () => {
    ...
    const report = auditDownDist10WalkForward({ bundle });
    expect(report.periods).toHaveLength(3);
    expect(report.periods.map((p) => p.id)).toEqual(['p2024_h2', 'p2025_h1', 'p2025_h2']);
  });
});
