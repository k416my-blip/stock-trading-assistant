/**
 * npx vitest run tests/unit/forwardValidationTakeProfitSensitivityAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditTakeProfitSensitivity } from '../../src/services/forwardValidation/forwardValidationTakeProfitSensitivityAudit';
...
describe('forwardValidationTakeProfitSensitivityAudit', () => {
  it('returns four take-profit scenarios', () => {
    ...
    const report = auditTakeProfitSensitivity({ bundle });
    expect(report.scenarios).toHaveLength(4);
    expect(report.scenarios.map((s) => s.takeProfitPct)).toEqual([3, 4, 5, 6]);
  });
});
