/**
 * npx vitest run tests/unit/forwardValidationVix25WinnerHoldAudit.test.ts
 */
import { describe, expect, it } from 'vitest';
import { auditVix25WinnerHold } from '../../src/services/forwardValidation/forwardValidationVix25WinnerHoldAudit';
...
describe('forwardValidationVix25WinnerHoldAudit', () => {
  it('returns 5 hold-day buckets for vix25 winners', () => {
    ...
    const report = auditVix25WinnerHold({ bundle });
    expect(report.buckets).toHaveLength(5);
    expect(report.buckets.reduce((s, b) => s + b.tradeCount, 0)).toBeLessThanOrEqual(report.winnerCount);
  });
});
