/**
 * buy + 出来高<1.2 — 52週安値距離 × ATR × 連続buy 3Dグリッド（n>=2）
 * npx vitest run tests/unit/openAiBuyVol12Low52AtrConsecGrid.test.ts
 */
...
const CONSEC_BANDS = [
  { id: '1_4', label: '連続1-4回', match: (n: number) => n >= 1 && n <= 4 },
  { id: '5_8', label: '連続5-8回', match: (n: number) => n >= 5 && n <= 8 },
  { id: '9plus', label: '連続9回以上', match: (n: number) => n >= 9 },
] as const;
...
describe('buy vol<1.2 low52 x ATR x consecutive 3d grid', () => {
  it(
    'writes 3d grid JSON (n>=2, top/worst20)',
    async () => {
      ...
      for (const [symbol, list] of bySymbol) {
        ...
        list.forEach((b, i) => consecByKey.set(`${b.date}|${symbol}`, i + 1));
      }
      ...
      for (const o of buys) {
        ...
        rows.push({
          date: o.date,
          symbol: o.symbol,
          consecutiveBuyNumber: consecByKey.get(`${o.date}|${o.symbol}`)!,
      ...
      const allCells = LOW52_BANDS.flatMap((low) =>
        ATR_BANDS.flatMap((atr) =>
          CONSEC_BANDS.map((con) => {
            const inCell = rows.filter(
              (r) =>
                low.match(r.pctFrom52wLow) &&
                atr.match(r.atrPct) &&
                con.match(r.consecutiveBuyNumber),
            );
            return {
              low52BandId: low.id,
              ...
              consecBandId: con.id,
              consecBandLabel: con.label,
              conditionLabel: `${low.label} × ${atr.label} × ${con.label}`,
              ...cellMetrics(inCell),
            };
          }),
        ),
      ...
      const report = {
        methodologyJa: {
          ...
          dimensions: [
            '52週安値距離: 0-3/3-5/5-8/8%+',
            'ATR: <1.5/1.5-2.0/2.0-2.5/>2.5',
            '連続buy: 1-4/5-8/9+（銘柄内通算）',
          ],
          displayRule: `n>=${MIN_N} のセルのみ表示・ランキング`,
          rankingMetric: '10営業日平均',
        },
        ...
        insightJa: [
          ...
          cellsMinN.length === 0
            ? `n>=${MIN_N}セル0件`
            : `n>=${MIN_N}セル${cellsMinN.length}件`,
        ],
      };

      const out = path.join(process.cwd(), 'scripts', 'openai-buy-vol12-3d-grid.json');
