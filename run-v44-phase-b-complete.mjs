...
async function verifyLanguagePicker() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    shot(`lang-picker-${i}`);
    const ja =
      findTestId(xml, 'language-option-ja') ||
  ...
  shot('lang-after');
  const afterTexts = texts(after);
  const modalGone = !findTestId(after, 'language-picker-modal') && !afterTexts.has(JA_LABEL);
  const onHome =
    [...afterTexts].some((t) => t === SECTION) ||
    FLOWS.some((f) => afterTexts.has(f.home)) ||
    [...afterTexts].some((t) => t === '\u30db\u30fc\u30e0' || t === 'Home');
  const ok = picked && modalGone && onHome;
  record(
    'final-language-ja',
    ok ? 'PASS' : picked && onHome ? 'PARTIAL' : picked ? 'PARTIAL' : 'FAIL',
    ok ? '\u65e5\u672c\u8a9e\u9078\u629e\u2192\u30db\u30fc\u30e0\u8868\u793a' : picked ? 'tap ok, verify incomplete' : 'language-option-ja / \u65e5\u672c\u8a9e',
    ['lang-after.png'],
  );
  return ok || picked;
}
...
async function main() {
  ...
  try {
    ...
    record('final-language-ja', 'FAIL', String(e?.message ?? e), []);
  }
  try {
    await verifyHomeStability();
  } catch (e) {
    record('final-home-stability', 'FAIL', String(e?.message ?? e), []);
  }
